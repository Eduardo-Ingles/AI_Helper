...
CpuCoreNumber = os.cpu_count() # - offset #offset = 0
...
def process_files(sio, client_id, data, script, **kwargs):
	...
	# galleria
	# files
    output = f"client_id: {client_id} -> {data}"
    ...
    for galleria in gallerie:          
        ...
        if(galleria and cpu_usage < 70 and percent_ram < 70):
            ...
            start = time.time()  
            fileAnagrafica = None
            for output in dragonflyPorter.mainCall(galleria, UploadsFileFolder, DownloadsFileFolder, fileAnagrafica, auxName, CpuCoreNumber, chosenDB, soloScarico = True, yieldingFlag = True):
                yield output
            end = time.time()
